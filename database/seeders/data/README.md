...
`iop-level-1.json` / `cra-level-1.json` / `eniripsa-level-1.json` / `sram-level-1.json` / `xelor-level-1.json` / `feca-level-1.json` / `osamodas-level-1.json` / `enutrof-level-1.json` : 6 sorts par classe (3 emplacements × 2 variantes). Upsert sur `dofusdb_id` ou `official_id` (`jdr:attaque-naturelle`, `jdr:oeil-de-lynx`). `auto_update = false`, état `playable`. `target_type` optionnel (`direct`, `trap`, `glyph` ; défaut `direct`, ex. Piège Sournois, Glyphe Enflammé). Les autres sorts liés à la classe passent hors grille (`character_level` 0, `slot_index` 1).
