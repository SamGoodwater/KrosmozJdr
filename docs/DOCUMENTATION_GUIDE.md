...
- `docs/IA/` : exception volontaire — cadrage de l’**IA générative métier** (LLM pour objets, sorts, monstres, PNJ). Config des champs figés : `resources/ia/generation.json`. Le pipeline LLM n’est pas branché ; les fichiers `_ai.md` ailleurs dans `/docs` orientent les agents de développement.
