...
| **ValidationService** | `app/Services/Scrapping/Core/Validation/ValidationService.php` | Valide les données converties contre CharacteristicService (BDD). |
...
- **CLI** : `php artisan scrapping --import=monster --id=31 [--validate-only] [--dry-run] [--json]` — utilise `Orchestrator::default()` → runOne / runMany.
- **API** : **POST /api/scrapping/import/{entity}/{id}** (ScrappingController) pour class, monster, spell, item. Import monster : runOne (entité principale Creature + Monster). Les relations (sorts, drops) peuvent être résolues via RelationResolutionService lorsque les données brutes incluent spells/drops (ex. appel explicite avec rawData + creatureId).
