...
Ce document consolide les informations sur l’API DofusDB nécessaires pour la refonte : endpoints, paramètres, **structure des réponses** (déduite des configs et de la doc existante), pagination et filtres. Il sert de référence pour les configs « requêtes » et « mapping ».
...
La commande `scrapping:v2` expose ces filtres de base : `--name`, `--level-min`, `--level-max`, `--id-min`, `--id-max`, `--race-id` (monstres), `--type-id` (objets). Ils sont convertis en paramètres Feathers dans le service de collecte.
...
Les **races de monstres**, **item-types** et **super-types** sont exposés comme entités collectables via la commande `scrapping:v2`, au même titre que les monstres ou les items :
...
php artisan scrapping:v2 --collect=monster-race --json

# Tous les item-types (232)
php artisan scrapping:v2 --collect=item-type --json

# Tous les superTypes uniques (dérivés des item-types)
php artisan scrapping:v2 --collect=item-super-type --json

# Limiter à 100 objets, ou commencer à l'offset 50
php artisan scrapping:v2 --collect=monster --limit=100 --offset=0 --json
```
...
Une fois ces vérifications faites, mettre à jour ce document et les configs V2 en conséquence.
