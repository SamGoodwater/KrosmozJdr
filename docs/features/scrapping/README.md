...
À l’intégration, le **prix Dofus n’est plus versé dans `price_custom`** des équipements : `EntityPriceRecalculator` écrit `price_calculated` (formule bonus + niveau + rareté). Les ressources conservent le prix Dofus dans `price`. Les consommables sont recalculés après attache de la recette (somme des ingrédients).
