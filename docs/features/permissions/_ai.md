...
- **Policies** : `BaseEntityPolicy` (admin → auteur → matrice affichage → state + read/write_level) ; surcharges par entité ; `UserPolicy`. `CreaturePolicy::viewResolvedStats` suit le `view` du monstre/PNJ lié (pas un bypass public). Sorts/équipements nested d’un monstre : `visibleToUser` à l’eager-load. Détail : [README](./README.md#policies).
