...
- **Projection front** : `EntityPermissionService` calcule les droits (cache 10 min) → partagés via `HandleInertiaRequests` → composable `usePermissions` (`can`, `canAccess`). `contentManagement` = `items`/`updateAny` (MJ+). Les droits **par ligne** viennent du champ `can` des Resources, pas de `usePermissions`.
