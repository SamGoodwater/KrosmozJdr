...
- **Policies** : `BaseEntityPolicy` (admin → auteur → matrice affichage → state + read/write_level) ; surcharges par entité ; `UserPolicy`. `CreaturePolicy::viewResolvedStats` suit le `view` du monstre/PNJ lié (pas un bypass public). Payload panoplie d’un équipement (`ItemPanoplyPayload`) : mêmes règles `view` / `visibleToUser` sur le set et ses pièces. Sorts/équipements nested d’un monstre : `visibleToUser` à l’eager-load. Fiche ressource : objets/consommables/campagnes liés filtrés `visibleToUser`. Détail : [README](./README.md#policies).
