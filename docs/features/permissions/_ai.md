...
- `app/Support/EntityPermissions/EntityPermissionService.php`, `app/Services/EntityDisplay/EntityDisplayVisibilityService.php` (`constrainQueryToViewer` pour listes ; colonnes `state`/`read_level`/`write_level`/`created_by` qualifiées par table, pour rester valides après un JOIN). Nested classes : `BreedController` / `BreedTableController` / `PdfService` appliquent `visibleToUser` aux sorts/capacités/traits/PNJ liés.
