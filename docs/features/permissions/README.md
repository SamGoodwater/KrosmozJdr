...
Surcharges notables : `SpellPolicy` (`updateAny` admin), `BreedPolicy` (logique propre, n'étend pas la base), `PagePolicy`/`SectionPolicy` (CMS via `canBeViewedBy`/`canBeEditedBy`), `UserPolicy` (`before` = super_admin interactif, `updateRole` restreint), `CreaturePolicy::viewResolvedStats` (stats JSON : `view` du monstre/PNJ lié, pas un accès public inconditionnel). Les relations embarquées suivent la même `view` : `ItemPanoplyPayload` (sets / pièces d’un équipement), les sorts/équipements liés d’un monstre, les sorts/capacités/traits/PNJ liés d’une classe, et les sorts/capacités/traits/objets/PNJ liés d’une spécialisation (`visibleToUser` à l’eager-load). Les endpoints publics `GET /api/effects/for-entity` et `GET /api/effects/usages` exigent aussi `view` sur la fiche parente. Enregistrement explicite partiel dans `app/Providers/AuthServiceProvider.php` ; le reste par auto-discovery Laravel.
