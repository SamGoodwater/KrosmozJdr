...
- **Menu dynamique** : `GET /pages/menu` (JSON) → `useDynamicMenu` → `DynamicMenu.vue`. Groupes dans `config/nav_menu.php` (L'Essentiel, Règles, Bibliothèques, Pour les MJ, Informations) ; un groupe sans enfants visibles n’apparaît pas. Bibliothèques = classes, spés, sorts, capacités, monstres, **PNJ**, équipements… Les sous-pages menu par classe / spécialisation (`pages:sync-bibliotheque-entities`) ne ciblent que les fiches **jouables**. L’Essentiel = aide-mémoire (`database/seeders/data/essential-pages.php`), aligné sur `private/game/rules`.
