...
- **Rendu front** : `PageRenderer` → `SectionLazyGate` (lazy) → `SectionRenderer` → template via registry auto-discovery. Largeur : cadre `w-full min-w-0` ; sections hors tableaux `w-full md:w-2/3` centrées ; templates `*_table` en `w-full` ; `overflow-x-auto` si le contenu dépasse. Détail : [README](./README.md#rendu-frontend).
