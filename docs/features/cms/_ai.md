...
- **Chapitre 5 règles** (`regles-5-*`) : équilibrage des entités. Import TOC → `menu_group=Pour les MJ`, `read_level` MJ. Hors PDF joueur.
- **Page « Ressources »** (`ressources-de-jeu`) : racine du menu Règles. Téléchargements (`download_catalog`, `config/game_downloads.php`). Compilation `rules:compile-downloads` (PDF A4 compact, saut de page par partie, sans chap. 5).
