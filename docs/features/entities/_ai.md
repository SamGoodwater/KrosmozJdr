...
- **Objets (vue Colonnes)** : image, nom, niveau, type, rareté, bonus (`items.bonus`) ; description et résumé masqués ; `state` si `updateAny`. En-têtes : `column.tooltip`. Filtre Type : types `show_in_catalog` précochés (flag admin, pas apparats/costumes par défaut). Recette (ressources liées) : catalogue / fiche lecture filtrés `visibleToUser` (un ingrédient brouillon ne fuit pas via un objet jouable) ; l’édition charge toute la recette.
