...
- **Monstres** : coquille `Monster` + stats/sorts/équipements/traits sur `Creature` ; sorts, équipements **et traits** liés filtrés `visibleToUser` ; PDF multi + `visibleToUser` ; Full affiche sorts/empty states, équipements seulement s’il y en a. `resolved-stats` : même visibilité que le monstre/PNJ lié, et objets brouillon exclus des totaux publics. Tri catalogue par nom via sous-requête (pas de JOIN `creatures`).
- **PNJ** : coquille `Npc` (rôle `npc_role`, taille 0–5 via `CreatureSize`, `story`/`historical`/`age`, langues `npc_language`, panoplies, boutique) + corps `Creature` (nom, `location` string, hostilité, niveau, stats, sorts `creature_spell`, stuff `creature_item`, traits). `state` de la coquille synchronisé sur la créature à la création / changement d’état. Pas de scrapping DofusDB. Kit porté : 1 objet / emplacement (2 anneaux) via `NpcEquipmentSlotValidator` ; types hors équipement refusés. Panoplies associées sans auto-remplir les slots. CRUD : `NpcController` (store crée la Creature si `name`) ; catalogue Index **server-side** (`NpcTableController`) ; page CMS **Bibliothèques → PNJ** (`bibliotheque-npc`, `config/nav_menu.php`) ; vues Full/Line/Minimal/Edit alignées Monster sans panneau DofusDB.
- **Classes (`breeds`)** : fiche / table / PDF **et page bibliothèque CMS** (`LinkedEntityShow`) filtrent sorts, capacités, traits et PNJ liés avec `visibleToUser`. Le menu Bibliothèques ne crée une sous-page que pour les classes **jouables**. L’édition charge toujours toutes les liaisons.
- **Spécialisations** : fiche / table / PDF **et page bibliothèque CMS** filtrent sorts, capacités, traits, objets, consommables, ressources et PNJ liés avec `visibleToUser`. Le menu ne liste que les spécialisations **jouables** (les brouillons restent au catalogue, filtrables par état). L’édition charge toujours toutes les liaisons.
...
- **Objets (vue Colonnes)** : image, nom, niveau, type, rareté, bonus (`items.bonus`) ; description et résumé masqués ; `state` si `updateAny`. En-têtes : `column.tooltip`. Filtre Type : types `show_in_catalog` précochés (flag admin, pas apparats/costumes par défaut). Recette (ressources liées) : catalogue / fiche lecture filtrés `visibleToUser` ; l’édition charge toute la recette.
- **Consommables** : filtre Type précoché via `show_in_catalog` (potions, nourritures, parchemins… ; pas certificats / coffres / fées d’artifice par défaut). Recette : même filtre `visibleToUser` que les objets (édition non filtrée).
- **Ressources, monstres, sorts** : mêmes filtres type / race précochés via `show_in_catalog` (`defaultByCatalog` + options API). Recette d’une ressource (`recipeIngredients`) : catalogue / fiche lecture filtrés `visibleToUser` ; l’édition charge tous les ingrédients.
