...
- **Monstres** : coquille `Monster` + stats/sorts/équipements sur `Creature` ; sorts, équipements **et traits** liés en `visibleToUser` (aperçu sorts = `effect_usages_chips`, pas l’arbre) ; PDF multi + `visibleToUser` ; Full affiche sorts/empty states, équipements seulement s’il y en a. `resolved-stats` créature : même visibilité que le monstre/PNJ lié **et** objets agrégés filtrés `visibleToUser` (un équipement brouillon n’entre pas dans les totaux publics). Tri catalogue par nom via sous-requête (pas de JOIN `creatures`).
