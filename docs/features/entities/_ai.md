...
- **PNJ** : coquille `Npc` (rôle `npc_role`, taille 0–5 via `CreatureSize`, `story`/`historical`/`age`, langues `npc_language`, panoplies, boutique) + corps `Creature` (nom, `location` string, hostilité, niveau, stats, sorts `creature_spell`, stuff `creature_item`, traits). `state` de la coquille synchronisé sur la créature à la création / changement d’état. Pas de scrapping DofusDB. Kit porté : 1 objet / emplacement (2 anneaux) via `NpcEquipmentSlotValidator` ; types hors équipement refusés. Panoplies associées sans auto-remplir les slots. CRUD : `NpcController` (store crée la Creature si `name`) ; catalogue Index **server-side** (`NpcTableController`, tri nom/stats via sous-requête créature, filtres hostilité + combat) ; vues Full/Line/Minimal/Edit alignées Monster sans panneau DofusDB.
- **Classes (`breeds`)** : fiche / table / PDF **et page bibliothèque CMS** (`LinkedEntityShow`) filtrent sorts, capacités, traits et PNJ liés avec `visibleToUser` (un sort brouillon n’apparaît pas sur une classe jouable). L’édition charge toujours toutes les liaisons.
- **Spécialisations** : fiche / table / PDF **et page bibliothèque CMS** filtrent sorts, capacités, traits, objets, consommables, ressources et PNJ liés avec `visibleToUser` (un sort brouillon n’apparaît pas sur une spécialisation jouable). L’édition charge toujours toutes les liaisons.
