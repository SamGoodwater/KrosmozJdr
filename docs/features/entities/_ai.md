...
- **Registres de types** : pages admin `/entities/item-types` (et resource/consumable/races/spells). `decision` = inclus scrap / maj de masse ; `show_in_catalog` = filtre catalogue (item/resource/consumable). Races / sorts : visibilité catalogue = `state` playable.
