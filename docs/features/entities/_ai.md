...
- **Registres de types** : page commune `/admin/content/types/{equipment|resource|consumable|race|spell}` (anciennes URLs `/entities/item-types` etc. redirigent). `decision` = scrap ; `show_in_catalog` = en jeu / filtre catalogue. Races / sorts : en jeu = `state` playable. Déplacement de catégorie seulement item/resource/consumable.
