...
- **Table** : `resources/js/Pages/Organismes/table/EntityTanStackTable.vue` (server-side) ; préférences/filtres via `resources/js/Composables/table/*`. Les en-têtes lisent `column.tooltip`. Catalogue objets : image, nom, niveau, type, rareté, bonus (`items.bonus`) ; `state` réservé aux éditeurs. Rareté 0–5 : Commun, Peu commun, Rare, Très rare, Légendaire, Unique (mêmes libellés filtres et vues). Panoplies : pièces en vue texte, vignette = images d’équipements ou initiales ; catalogue et fiche lecture ne sérialisent que les pièces `visibleToUser` (l’édition charge toutes les pièces liées). Un équipement du set affiche `ItemPanoplyMark` (payload `panoplies` via `ItemPanoplyPayload`). Édition panoplie : équipements puis bonus en tête (cartes du formulaire, `PanoplyBonusEditor`) ; droits en bas. Recherche catalogue (`EntityPickerCore`).
