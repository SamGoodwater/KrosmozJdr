...
- **Aperçu sort depuis un monstre** : sorts liés avec `effect_usages_chips` (`SpellNestedPreviewSerializer`) ; clic → `SpellViewMinimal` étendu (effets + dropdown d’options haut gauche). Fetch `api.tables.spells` seulement si les chips manquent.
