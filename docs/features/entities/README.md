...
- **Vues** : 5 vues canoniques résolues par `resources/js/Utils/entity/resolveEntityViewComponent.js` :
  ...
  Les composants par type sont dans `resources/js/Pages/Molecules/entity/<type>/`. Sorts et capacités (Minimal / Line) : fond glass teinté par l’élément, dégradé si plusieurs primaires. Notes de règles : `spellTypeRuleNotes` (sorts), `consumableRuleNotes` / `itemRuleNotes` + `EntityRuleNotes`.
