...
- Sécurité : valider les entrées, ne pas versionner `.env`, garder les actions admin sous confirmation de mot de passe si sensibles. Les CVE transitives Node se corrigent via `pnpm.overrides` dans `package.json` (ex. `undici` 6.28.0) plutôt qu’en patchant le code applicatif. Dependabot versionne npm, Composer et GitHub Actions via `.github/dependabot.yml` (groupes weekly minor/patch ; un bump majeur de Vitest se fait à part).
