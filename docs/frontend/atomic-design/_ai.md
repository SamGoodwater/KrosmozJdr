...
- Tooltips hover (`Tooltip` / `OverlayTrigger`) : le panneau capte le pointeur (pont CSS `overlay-hover-bridge` + délai de fermeture). Le survol du tooltip ne le ferme pas. Sur tactile (`hover: none` / `pointer: coarse`), le mode passe en **clic** + fermeture hors panneau (`canUseHoverOverlay`) — évite les tooltips collants sur fiches d’entité. Une seule surface : pas de `tooltip-floating-surface` empilé si `chromeless` / `glass=false` / `panelClass` déjà chromé.
