...
- Tableaux d’entités : filtres auto-appliqués (server-side), `filters[k][]` (listes) ou `filters[k][min]` / `[max]` (plages), tri via `sort.field` / alias SQL, recherche `search=` ou nom côté client. Charte : **niveau** = slider dual natif (`RangeDualCore`, bornes catalogue) ; **état** = pastilles à point à droite de « Filtres » (défaut `playable` ; conditions : hors `raw`) ; types / rareté = menu select compact (`resolveTableFilterLayout`, défaut `menu`) ; stats combat (PA/PO, caracs créature) en filtres avancés range, **icône + couleur** caractéristique (`filterCharacteristicMeta`, `icons/caracteristics/`) ; **pas d’input texte**. Défauts de colonnes (`defaultByLabel`) seulement quand les options de **ce** filtre arrivent ; `initialFilterValues` fusionne par contenu, sans réécrire la saisie en cours.
- Catalogue objets / ressources / consommables / monstres / sorts : filtres Type ou Race précochés via `show_in_catalog` (`defaultByCatalog`). Les listes `GAMEPLAY_*` restent un repli si le flag est absent du payload.
- Action `refresh` (MJ+) : `POST /api/entities/{type}/{id}/dofusdb-refresh` (id local, types scrapables uniquement). Masse = atelier admin.
...
- Notes de règles (rappels courts) : sorts via `spellTypeRuleNotes` + `SpellUsageBlock` (`data-cy="spell-rule-notes"`) ; consommables / objets via `consumableRuleNotes` / `itemRuleNotes` + `EntityRuleNotes`. Toutes les vues (minimal compact+déployé, line, full).
...
- Sorts / capacités Minimal et Line : le glass (`EntityMinimalCard` `surfaceStyle`) garde le fond du thème ; l’élément se lit en **bordure** (`--element-border-color`, dégradé `--element-border-image` si plusieurs primaires, classe `entity-element-ring`). Sans élément, bordure thème. Couleurs : jetons DaisyUI (`--color-shop-700` Terre, `--color-error-600` Feu, etc.).
...
- **Éditer (toutes entités)** : raccourci des options → page Modifier (`edit`). Pas de modal ni de panneau d’édition multiple dans le tableau. Sélection : cases visibles dans les 3 vues ; `normalizeTableRowId`. Raccourcis tableau : `useTanStackTableKeyboard.js`.
