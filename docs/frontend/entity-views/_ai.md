...
- Tableaux d’entités : filtres auto-appliqués (server-side), `filters[k][]`, tri via `sort.field` / alias SQL, recherche `search=` ou nom côté client. Filtres : menus compactes (libellé dans le bouton), toggle pour booléen, champ texte sinon (`TanStackTableFilters`, `resolveTableFilterLayout`). Recherche dans le menu si > 8 options. Défauts de colonnes (`defaultByLabel`) seulement quand les options de **ce** filtre arrivent ; `initialFilterValues` fusionne par contenu, sans réécrire la saisie en cours.
