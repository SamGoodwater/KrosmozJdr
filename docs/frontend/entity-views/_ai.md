...
- Tableaux d’entités : filtres auto-appliqués (server-side), `filters[k][]`, tri via `sort.field` / alias SQL, recherche `search=` ou nom côté client. Layout filtres : `flex` (largeur au contenu) ; ≤ 8 options → chips, listes longues → menu (`resolveTableFilterLayout`, `filter.ui.layout`). Défauts de colonnes (`defaultByLabel`) seulement quand les options de **ce** filtre arrivent ; `initialFilterValues` fusionne par contenu, sans réécrire la saisie en cours.
...
- Sorts / capacités Minimal et Line : le glass (`EntityMinimalCard` `surfaceStyle`, `--bg-color`) prend la couleur d’élément ; multi-primaires → `background-image` dégradé (`getElementGlassSurfaceStyle`). Sans élément, fond glass par défaut.
