...
- Tableaux d’entités : filtres auto-appliqués (server-side), `filters[k][]` (listes) ou `filters[k][min]`/`[max]` (plages), tri via `sort.field` / alias SQL, recherche `search=` ou nom côté client. Layout filtres : `flex` (largeur au contenu) ; listes en menu compact (`resolveTableFilterLayout`) ; **Niveau** en slider dual inline (`RangeDualCore`) sur tous les catalogues qui en ont un ; stats numériques (PA / PO sorts & capacités ; PA / PO / PM + caracs créature monstres ; PA / PO / PM PNJ) dans les filtres avancés, icône + couleur BDD (`filterCharacteristicMeta` accepte singulier et pluriel, `icons/caracteristics/`). Un seul menu ouvert à la fois (`dropdownExclusive`). Défauts de colonnes (`defaultByLabel`) seulement quand les options de **ce** filtre arrivent ; `initialFilterValues` fusionne par contenu, sans réécrire la saisie en cours. État : badges à points à droite de « Filtres », défaut **Jouable**. Presets en BDD (`table_filter_presets`) : privés par utilisateur, publics créables par MJ/admin.
...
- Catalogue états : filtre État précoché **Jouable** (`CONDITION_CATALOG_STATE_DEFAULT` / `ENTITY_STATE_CATALOG_DEFAULT`) ; API exclut les autres si le filtre est absent. Flags mécaniques en chips (`ConditionMechanicalFlags`) + colonne Effets. Éditeur d’effets : picker hors `raw` (canons JDR).
- Overlay sort depuis un monstre : payload `effect_usages_chips` + `SpellViewMinimal` étendu (actions à droite du titre, overflow dropdown). Eager-load `visibleToUser` (brouillon masqué). Fetch `whitelist[]=` seulement si les chips manquent. Nested : `allows_reaction` + `auto_success_if_willing_target`.
- Notes de règles sorts (`spellTypeRuleNotes` → `SpellUsageBlock`) : Minimal (compact + déployé), Line, Full (Utilisation + Effets). Signaux : type/effet Soin, Invocation, `target_type` trap/glyph, `life_steal_formula`, `auto_success_if_willing_target`, `allows_reaction`, action `donner-pv-temporaires`. Pas de doublon sur le `parts="effects"` Minimal/Line.
