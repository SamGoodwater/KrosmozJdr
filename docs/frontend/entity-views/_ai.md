...
- **PNJ** : mêmes listes sorts/équipements (prop `creature`) ; Full = `EntityViewHeader` + lieu / rôle / langues / traits / récit / sorts (empty state) / stuff / panoplies / boutique ; Line = classe, rôle, taille, hostilité, lieu ; Minimal compact = lieu + rôle ; pas de panneau DofusDB. Descriptors : `npc_role`, `size` enum 0–5, `creature_location`, placeholder « Rechercher un PNJ ».
