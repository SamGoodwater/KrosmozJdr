...
- Overlay fiche minimale (`EntityViewTextLink`, recherche) : panneau `chromeless` — pas de seconde boîte tooltip autour de la carte. `displayMode: extended` n’affiche que le slot expanded (hauteur du contenu). Actions sans fond.
...
- **Panoplies** : pas d’image propre — vignette = images des équipements (sinon initiales du nom). Pièces en vue texte (`EntityViewTextLink` → `ItemViewMinimal`). Bonus `{ "2": {…} }` aplatis en chips (`2p Force`). Édition : pièces puis bonus de set en tête de page (cartes `bg-base-100/30`) ; nom / droits en bas. Picker API + `PanoplyBonusEditor` (caractéristique + valeur par palier).
