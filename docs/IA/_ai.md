...
- **Socle rejouable** : `database/seeders/data/entities/items/*-item.json` (1 fichier/item) + `database/seeders/data/entities/panoplies/*-panoply.json`. `items:seeder-export` / `items:seeder-import` ; `Entity\ItemSeeder` puis `Entity\ResourceSeeder` (recettes → `playable`) puis `Entity\PanoplySeeder` dans `project:seed`. Boutons super_admin items sur `/admin/content/ia-generation`.
