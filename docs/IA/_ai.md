...
- **UI** : un modal `EntitySourceModal` (DofusDB | IA), une icône « Sources ». Après scrap ou conversion IA : tableau **avant / après** (`EntityUpdateDiffView`) — Enregistrer garde, Rétablir restaure l’instantané. Volet IA : admin+ **et** `password.confirm` (même fenêtre que la gestion admin). Solde : tokens du mois (`ai_generation_runs`) + crédit Anthropic s’il est connu, dans la page admin **et** le modal. HTTP `ia-convert` / `ia/status` = `role:admin` + `password.confirm`. Clé absente = 422 honnête, aucun run.
