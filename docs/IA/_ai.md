...
- **Socle rejouable** : `database/seeders/data/entities/items/*-item.json` (1 fichier/item) + `database/seeders/data/entities/panoplies/*-panoply.json` + `database/seeders/data/entities/consumables/healing-out-of-combat.json`. `items:seeder-export` / `items:seeder-import` ; `Entity\ItemSeeder` puis `Entity\ResourceSeeder` (recettes → `playable`, plancher 1 kama) puis `Entity\ConsumableSeeder` puis `Entity\PanoplySeeder` dans `project:seed`. Boutons super_admin items sur `/admin/content/ia-generation`.
