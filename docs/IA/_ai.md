...
- **UI** : un modal `EntitySourceModal` (onglets Conversion DofusDB | Conversion IA | **JSON**). Après scrap, conversion ou injection : tableau **avant / après** — clic cellule ou en-tête pour garder l’ancienne ou la nouvelle valeur ; **Enregistrer** applique le mix (`update-diff/apply`) ; **Rétablir** / fermer annule (`restore`). Volet DofusDB : cases contenu + **Récupérer l’image**. Volet IA : admin+ **et** types convertibles + `password.confirm`. Volet **JSON** : admin+ sur **tous** les types du registre (spécialisation IA si convertible, sinon fillable générique → `auto`) + `password.confirm`. Solde : tokens du mois (`ai_generation_runs`) + crédit Anthropic s’il est connu, dans la page admin **et** le modal. HTTP `ia-convert` / `ia-inject` / `ia/status` / `ia/schema/{type}` = `role:admin` + `password.confirm`. Clé absente = 422 pour `ia-convert` seulement ; `ia-inject` n’appelle pas Anthropic. `action` calée sur le type d’URL pour les convertibles ; `playable`/`archived` exigent `force`. **Création** : `CreateEntityModal` — à la main, JSON (admin, tous types), ou Conversion IA (admin, convertibles) ; stub `store` puis `ia-inject`.
