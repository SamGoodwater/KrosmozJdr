...
- Import JSON : `ConversionPipeline::runFromPayload` + `ManualJsonPayloadSanitizer` (max 256 Ko, profondeur 12, `strip_tags` sur les chaînes). HTTP `POST …/ia-inject` (mêmes gates que `ia-convert`, **sans** clé Anthropic). Schéma d’exemple : `GET /api/ia/schema/{entityType}`.
...
- HTTP : `POST /api/entities/{type}/{id}/ia-convert` et `…/ia-inject` (`role:admin`, `password.confirm`, throttle 12/min). Types : `monsters`, `spells`, `npcs`, `items`, `consumables`. L’`action` doit correspondre au type d’URL. Fiche `playable` / `archived` : `force=true`. Statut : `GET /api/ia/status` ; schéma : `GET /api/ia/schema/{type}`. Instantané : `POST …/update-diff/restore`.
...
- UI : « Sources » → `EntitySourceModal` (DofusDB | Conversion IA | **JSON**). Après écriture : tableau avant/après. Création : `CreateEntityModal` (manuel | IA | JSON).
