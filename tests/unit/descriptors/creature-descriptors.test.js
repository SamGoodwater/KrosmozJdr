/**
 * Tests unitaires pour creature-descriptors
 *
 * @description
 * Vérifie que :
...
describe('creature-descriptors', () => {
    describe('Structure des descriptors', () => {
        it('retourne un objet avec tous les champs requis', () => {
            const descriptors = getCreatureFieldDescriptors();
            const requiredFields = ['id', 'name', 'level', 'hostility', 'life', 'usable', 'is_visible'];
    ...
    describe('QUICK_EDIT_FIELDS', () => {
        it('QUICK_EDIT_FIELDS est défini et est un tableau', () => {
            expect(CREATURE_QUICK_EDIT_FIELDS).toBeDefined();
            expect(Array.isArray(CREATURE_QUICK_EDIT_FIELDS)).toBe(true);
        });

        it('QUICK_EDIT_FIELDS contient des champs valides', () => {
            const descriptors = getCreatureFieldDescriptors();
            CREATURE_QUICK_EDIT_FIELDS.forEach((field) => {
                expect(descriptors).toHaveProperty(field);
            });
        });
    });
