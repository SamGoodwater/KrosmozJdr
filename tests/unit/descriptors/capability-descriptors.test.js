...
describe('capability-descriptors', () => {
    describe('Structure des descriptors', () => {
        it('retourne un objet avec tous les champs requis', () => {
            ...
            const requiredFields = ['id', 'name', 'level', 'pa', 'po', 'usable', 'is_visible'];
    ...
    describe('QUICK_EDIT_FIELDS', () => {
        it('QUICK_EDIT_FIELDS est défini et est un tableau', () => {
            expect(CAPABILITY_QUICK_EDIT_FIELDS).toBeDefined();
            expect(Array.isArray(CAPABILITY_QUICK_EDIT_FIELDS)).toBe(true);
        });

        it('QUICK_EDIT_FIELDS contient des champs valides', () => {
            const descriptors = getCapabilityFieldDescriptors();
            CAPABILITY_QUICK_EDIT_FIELDS.forEach((field) => {
                expect(descriptors).toHaveProperty(field);
            });
        });
    });
