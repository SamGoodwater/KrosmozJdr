/**
 * Tests unitaires pour consumable-descriptors
 *
 * @description
 * Vérifie que :
...
describe('consumable-descriptors', () => {
    ...
    describe('QUICK_EDIT_FIELDS', () => {
        it('QUICK_EDIT_FIELDS est défini et est un tableau', () => {
            expect(CONSUMABLE_QUICK_EDIT_FIELDS).toBeDefined();
            expect(Array.isArray(CONSUMABLE_QUICK_EDIT_FIELDS)).toBe(true);
        });

        it('QUICK_EDIT_FIELDS contient des champs valides', () => {
            const descriptors = getConsumableFieldDescriptors();
            CONSUMABLE_QUICK_EDIT_FIELDS.forEach((field) => {
                expect(descriptors).toHaveProperty(field);
            });
        });
    });
