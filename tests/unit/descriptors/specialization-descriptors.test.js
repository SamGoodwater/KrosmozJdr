...
describe('specialization-descriptors', () => {
    ...
    describe('QUICK_EDIT_FIELDS', () => {
        it('QUICK_EDIT_FIELDS est défini et est un tableau', () => {
            expect(SPECIALIZATION_QUICK_EDIT_FIELDS).toBeDefined();
            expect(Array.isArray(SPECIALIZATION_QUICK_EDIT_FIELDS)).toBe(true);
        });

        it('QUICK_EDIT_FIELDS contient des champs valides', () => {
            const descriptors = getSpecializationFieldDescriptors();
            SPECIALIZATION_QUICK_EDIT_FIELDS.forEach((field) => {
                expect(descriptors).toHaveProperty(field);
            });
        });
    });
