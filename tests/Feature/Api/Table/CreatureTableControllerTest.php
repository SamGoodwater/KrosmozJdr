...
/**
 * Tests Feature pour CreatureTableController
 *
 * @description
 * Vérifie que :
 ...
 */
class CreatureTableControllerTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_format_entities_returns_raw_data(): void
    {
        ...
        $response = $this->actingAs($user)
            ->getJson('/api/tables/creatures?format=entities&limit=10');
        ...
        $this->assertEquals('Test Creature', $data['entities'][0]['name']);
    }
    ...
    public function test_format_cells_returns_formatted_cells(): void
    {
        ...
        $response = $this->actingAs($user)
            ->getJson('/api/tables/creatures?limit=10');
    ...
    {
        $user = User::factory()->create();
        Creature::factory()->count(15)->create();

        $response = $this->actingAs($user)
            ->getJson('/api/tables/creatures?format=entities&limit=5');
    ...
    {
        $user = User::factory()->create();
        Creature::factory()->create(['name' => 'Dragon']);
        Creature::factory()->create(['name' => 'Goblin']);
        Creature::factory()->create(['name' => 'Orc']);

        $response = $this->actingAs($user)
            ->getJson('/api/tables/creatures?format=entities&search=Dragon&limit=10');
        ...
        $this->assertTrue(
            collect($data['entities'])->contains(fn ($e) => str_contains($e['name'], 'Dragon'))
        );
    }
    ...
    {
        $user = User::factory()->create();
        Creature::factory()->create(['name' => 'Z Creature', 'level' => '1']);
        Creature::factory()->create(['name' => 'A Creature', 'level' => '10']);

        $response = $this->actingAs($user)
            ->getJson('/api/tables/creatures?format=entities&sort=name&order=asc&limit=10');

        $response->assertOk();

        $data = $response->json();
        $this->assertCount(2, $data['entities']);
        $this->assertEquals('A Creature', $data['entities'][0]['name']);
        $this->assertEquals('Z Creature', $data['entities'][1]['name']);
    }
}
