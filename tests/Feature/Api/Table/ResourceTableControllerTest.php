...
/**
 * Tests Feature pour ResourceTableController
 *
 * @description
 * Vérifie que :
 ...
 */
class ResourceTableControllerTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_format_entities_returns_raw_data(): void
    {
        ...
        $response = $this->actingAs($user)
            ->getJson('/api/tables/resources?format=entities&limit=10');
        ...
        $this->assertEquals('Test Resource', $data['entities'][0]['name']);
    }
    ...
    public function test_format_cells_returns_formatted_cells(): void
    {
        ...
        $response = $this->actingAs($user)
            ->getJson('/api/tables/resources?limit=10');
    ...
    public function test_entities_format_respects_permissions(): void
    {
        ...
        Resource::factory()->create(['created_by' => $user->id]);
    ...
    {
        $user = User::factory()->create();
        Resource::factory()->count(10)->create(['created_by' => $user->id]);

        $response = $this->actingAs($user)
            ->getJson('/api/tables/resources?format=entities&limit=5');
