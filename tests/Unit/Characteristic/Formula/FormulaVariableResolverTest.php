...
class FormulaVariableResolverTest extends TestCase
{
    ...
    {
        $full = ['level_creature' => 5, 'life_points_creature' => 100];
        $result = FormulaVariableResolver::withShortNames('creature', $full);
        $this->assertSame(5, $result['level_creature']);
        $this->assertSame(100, $result['life_points_creature']);
        $this->assertSame(5, $result['level']);
        $this->assertSame(100, $result['life_points']);
    }
    ...
    public function test_key_to_short_name_strips_suffix(): void
    {
        ...
        $this->assertSame('life_points', FormulaVariableResolver::keyToShortName('life_points_creature'));
        $this->assertSame('life_points_max', FormulaVariableResolver::keyToShortName('life_points_max_object'));
        $this->assertSame('action_points', FormulaVariableResolver::keyToShortName('action_points_spell'));
    }
