import { decodeKrefTitle, parseKrefPayload } from "@/Composables/richText/ReferenceInlineExtension";
...
export function isKrefNavigableType(krefType) {
    return NAVIGABLE_TYPES.has(String(krefType || ""));
}
...
export function getDecodedKrefFromElement(el) {
    ...
    const decoded = decodeKrefTitle(span.getAttribute("title") || "");
...
export function getSectionIdForPreview(info) {
    if (String(info?.krefType || "") !== "pageSection") return null;
