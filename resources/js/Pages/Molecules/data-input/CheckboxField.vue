 ...
 *
 * @description
 * Molecule pour case à cocher complète, orchestrant CheckboxCore (atom), InputLabel, validation, actions contextuelles, etc.
 * - API simplifiée : prop `label` (string ou objet), validation, helper, actions
 * - 4 positions de labels : top, bottom, start, end (top par défaut)
 ...
 *
 * @example
 * <CheckboxField label="Accepter les CGU" v-model="accepted" :actions="['reset']" />
 * <CheckboxField :label="{ start: 'Option', end: 'Info' }" v-model="checked" />
 *
 * @props {String|Object} label - Label simple (string) ou objet avec positions (top, bottom, start, end)
 ...
 * @slot labelTop, labelBottom, labelStart, labelEnd
...
import { getCheckboxProps, getCheckboxAttrs } from '@/Utils/atomic-design/inputHelper';
