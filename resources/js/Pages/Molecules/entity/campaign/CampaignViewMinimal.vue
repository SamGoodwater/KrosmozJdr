...
const entity = computed(() => props.campaign);
...
const getCell = (fieldKey) =>
    props.campaign.toCell(fieldKey, { size: "sm", context: "minimal" });
...
const tooltipForField = (fieldKey, cell) => {
    const value =
        cell?.value === null || typeof cell?.value === "undefined" || String(cell?.value) === ""
            ? "-"
            : cell.value;
    const label = getEntityFieldShortLabel(
        fieldKey,
        descriptors.value?.[fieldKey]?.general?.label || fieldKey,
    );
...
const {
    ...
    entityTypePlural: "campaigns",
    showRoute: "entities.campaigns.show",
    editRoute: "entities.campaigns.edit",
    routeParam: "campaign",
    emit,
...
const displayName = computed(() => props.campaign?.name || "Campagne");
const imageUrl = computed(() => {
    const u = props.campaign?.image ?? props.campaign?._data?.image;
