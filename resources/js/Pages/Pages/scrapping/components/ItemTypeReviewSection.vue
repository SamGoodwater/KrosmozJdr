<script setup>
/**
 * Section UX: revue des nouveaux typeId DofusDB détectés pour les équipements/objets.
 *
...
const loadPending = async () => {
    loading.value = true;
    try {
        const response = await fetch("/api/scrapping/item-types/pending", {
...
const updateDecision = async (itemTypeId, decision) => {
    ...
    try {
        const response = await fetch(`/api/scrapping/item-types/${itemTypeId}/decision`, {
            method: "PATCH",
            headers: {
        ...
        if (response.ok && data.success) {
            ...
            if (decision !== "pending") {
                items.value = items.value.filter((i) => i.id !== itemTypeId);
            }
        } else {
...
<template>
    <Card class="p-6">
        <div class="flex items-start justify-between gap-4 mb-4">
            <div>
                <h2 class="text-xl font-bold text-primary-100">Types DofusDB détectés (Objets/Équipements)</h2>
                <p class="text-sm text-primary-300 mt-1">
                    Ces typeId ont été détectés pendant le scrapping des items. Indique si on les utilise.
                </p>
            </div>
