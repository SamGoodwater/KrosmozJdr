...
const { tableQuickEditEnabled, onUpdateTableQuickEdit } = useEntityIndexQuickEditTable(Shop);
...
const { handleKeyboardIntent } = useEntityIndexTableIntents({
    ModelClass: Shop,
    routeShowName: "entities.shops.show",
    routeShowParam: "shop",
    ...
    openEdit: (model) => {
        if (!model?.id) return;
        router.visit(route('entities.shops.edit', { shop: model.id }));
    },
});
...
<template>
    ...
    <div class="space-y-6 pb-8 w-full">
        ...
        >
            <div class="min-w-0 overflow-x-auto">
                <EntityTanStackTable
                    ...
                    :response-adapter="getEntityResponseAdapter('shops')"
        ...
        <CreateEntityModal
            ...
            entity-type="shop"
            :create-allow-field-keys="getEntityCreateAllowFieldKeys('shops')"
