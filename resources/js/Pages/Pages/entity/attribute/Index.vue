...
import { createAttributesTanStackTableConfig } from './attributes-tanstack-table-config';
...
const tableConfig = computed(() => createAttributesTanStackTableConfig());
const serverUrl = computed(() => `${route('api.tables.attributes')}?limit=5000&_t=${refreshToken.value}`);
...
const handleRowDoubleClick = (row) => {
    ...
    if (!raw) return;
    const model = Attribute.fromArray([raw])[0] || null;
    if (!model) return;
