...
import { createScenarioTableConfig } from "@/Entities/scenario/ScenarioTableConfig";
...
const tableConfig = computed(() => {
    ...
    return createScenarioTableConfig(ctx);
});
...
const handleTableAction = async (actionKey, entity, row) => {
    ...
    const model = Scenario.fromArray([targetEntity])[0] || null;
    if (!model) return;
