...
const { tableQuickEditEnabled, onUpdateTableQuickEdit } = useEntityIndexQuickEditTable(Scenario);
...
const { handleKeyboardIntent } = useEntityIndexTableIntents({
    ModelClass: Scenario,
    routeShowName: "entities.scenarios.show",
    routeShowParam: "scenario",
    ...
    openEdit: (model) => {
        if (!model?.id) return;
        router.visit(route('entities.scenarios.edit', { scenario: model.id }));
    },
});
...
<template>
    ...
    <div class="space-y-6 pb-8 w-full">
        ...
        <CreateEntityModal
            ...
            :create-allow-field-keys="getEntityCreateAllowFieldKeys('scenarios')"
