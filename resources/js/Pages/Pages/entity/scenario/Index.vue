...
import { createScenariosTanStackTableConfig } from './scenarios-tanstack-table-config';
...
const tableConfig = computed(() => createScenariosTanStackTableConfig());
...
const handleBulkUpdate = async (payload) => {
  await bulkPatchJson('/api/entities/scenarios/bulk', payload);
  refreshToken.value++;
};
...
const handleRowDoubleClick = (row) => {
    ...
    if (!raw) return;
    const model = Scenario.fromArray([raw])[0] || null;
    if (!model) return;
        ...
        <div class="xl:grid xl:grid-cols-[minmax(0,1fr)_380px] xl:gap-6">
            <div class="min-w-0 overflow-x-auto">
                <EntityTanStackTable
                    ...
                    :response-adapter="adaptScenarioEntitiesTableResponse"
