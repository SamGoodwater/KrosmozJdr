...
const { handleKeyboardIntent } = useEntityIndexTableIntents({
    ...
    routeShowName: "entities.conditions.show",
    routeShowParam: "condition",
    ...
    openEdit: (model) => {
        if (!model?.id) return;
        router.visit(route('entities.conditions.edit', { condition: model.id }));
    },
});
...
<template>
    ...
    <div class="space-y-6 pb-8 w-full">
        ...
        <CreateEntityModal
            ...
            :create-allow-field-keys="getEntityCreateAllowFieldKeys('conditions')"
