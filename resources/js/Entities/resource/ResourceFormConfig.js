/**
 * ResourceFormConfig — Configuration des formulaires pour l'entité Resource
 *
 * @description
 * Configuration des formulaires d'édition (simple et bulk) pour les ressources.
 ...
 *
 * @param {Object} [ctx] - Contexte (permissions, resourceTypes, etc.)
 * @returns {FormConfig} Configuration des formulaires
 */
export function createResourceFormConfig(ctx = {}) {
  ...
  formConfig
    ...
    .addField(
      new FormFieldConfig({
        ...
        type: "select",
        label: descriptors.resource_type_id?.edit?.form?.label || descriptors.resource_type_id?.label || "Type de ressource",
      })
    ...
    .addField(
      new FormFieldConfig({
        key: "weight",
        type: "text",
        label: descriptors.weight?.edit?.form?.label || descriptors.weight?.label || "Poids",
      })
        .withGroup(descriptors.weight?.edit?.form?.group || "Métadonnées")
        .withShowInCompact(true)
