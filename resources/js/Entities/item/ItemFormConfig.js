/**
 * ItemFormConfig — Configuration des formulaires pour l'entité Item
 *
 * @description
 * Configuration des formulaires d'édition (simple et bulk) pour les items.
...
/**
 * Crée la configuration des formulaires pour Item
 *
 * @param {Object} [ctx] - Contexte (permissions, itemTypes, etc.)
 * @returns {FormConfig} Configuration des formulaires
 */
export function createItemFormConfig(ctx = {}) {
  const can = ctx?.capabilities || ctx?.meta?.capabilities || {};
  const canUpdateAny = Boolean(can?.updateAny);
  const itemTypes = Array.isArray(ctx?.itemTypes)
    ? ctx.itemTypes
    : Array.isArray(ctx?.meta?.itemTypes)
    ? ctx.meta.itemTypes
    : [];
  ...
  formConfig
    ...
    .addField(
      new FormFieldConfig({
        key: "item_type_id",
        type: "select",
        label: descriptors.item_type?.edit?.form?.label || descriptors.item_type?.label || "Type d'item",
      })
        .withGroup(descriptors.item_type?.edit?.form?.group || "Métier")
        .withHelp(descriptors.item_type?.edit?.form?.help || "Définit le type de l'item.")
        .withShowInCompact(true)
        .withOptions(descriptors.item_type?.edit?.form?.options || (() => [{ value: "", label: "—" }, ...itemTypes.map((t) => ({ value: t.id, label: t.name }))]))
        .withBulk(descriptors.item_type?.edit?.form?.bulk || {
          enabled: true,
          nullable: true,
