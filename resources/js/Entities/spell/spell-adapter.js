/**
 * Spell adapter (Option B)
 *
 * @description
 * Transforme une réponse backend "entities" en `TableResponse` conforme à TanStackTable v2,
 * en générant les `cells` côté frontend à partir de `spell-descriptors`.
 */

import { DEFAULT_SPELL_FIELD_VIEWS, getSpellFieldDescriptors } from "@/Entities/spell/spell-descriptors";
...
const spellShowHref = (id) => {
  try {
    return route("entities.spells.show", { spell: id });
  } catch {
    return null;
  }
};

const dofusDbSpellHref = (dofusdbId) => {
  const n = toNumber(dofusdbId);
  if (!n) return null;
  return `https://www.dofus.com/fr/mmorpg/encyclopedie/sorts/${n}`;
};

const resolveViewConfigFor = (descriptor, { view = "table" } = {}) => {
  const views = descriptor?.display?.views || DEFAULT_SPELL_FIELD_VIEWS;
...
/**
 * Construit une Cell v2 depuis une entité Spell brute.
 *
 * @param {string} colId
 * @param {any} entity
 * @param {any} ctx
...
export function buildSpellCell(colId, entity, ctx = {}, opts = {}) {
  ...
  if (colId === "area") {
    const v = entity?.area ?? null;
    const sortValue = toNumber(v) ?? 0;
    return {
      type: "text",
  ...
  if (colId === "spell_types") {
    const types = entity?.spellTypes || [];
    const typesLabel = Array.isArray(types) && types.length > 0
      ? types.map((t) => t?.name || "").filter(Boolean).join(", ")
      : "-";
    return {
      type: "text",
      value: typesLabel,
      params: {
        tooltip: typesLabel === "-" ? "" : typesLabel,
        truncate: { context, scale: truncateScale },
        searchValue: typesLabel === "-" ? "" : typesLabel,
  ...
  if (colId === "dofusdb_id") {
    ...
    const href = v ? dofusDbSpellHref(v) : null;
    return {
      type: "route",
      value: v ? String(v) : "-",
      params: {
...
export function adaptSpellEntitiesTableResponse({ meta, entities }) {
  ...
  const rows = (Array.isArray(entities) ? entities : []).map((entity) => {
    ...
    for (const colId of colIds) {
      cells[colId] = buildSpellCell(colId, entity, ctx, { context: "table" });
    }
    return {
      id: entity?.id ?? null,
      cells,
