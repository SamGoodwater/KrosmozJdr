/**
 * Campaign adapter (Option B)
 *
 * @description
 * Transforme une réponse backend "entities" en `TableResponse` conforme à TanStackTable v2,
 * en générant les `cells` côté frontend à partir de `campaign-descriptors`.
 */

import { DEFAULT_CAMPAIGN_FIELD_VIEWS, getCampaignFieldDescriptors } from "@/Entities/campaign/campaign-descriptors";
...
const campaignShowHref = (id) => {
  try {
    return route("entities.campaigns.show", { campaign: id });
  } catch {
    return null;
  }
};
...
const resolveViewConfigFor = (descriptor, { view = "table" } = {}) => {
  const views = descriptor?.display?.views || DEFAULT_CAMPAIGN_FIELD_VIEWS;
...
/**
 * Construit une Cell v2 depuis une entité Campaign brute.
 *
 * @param {string} colId
 * @param {any} entity
 * @param {any} ctx
 ...
 */
export function buildCampaignCell(colId, entity, ctx = {}, opts = {}) {
  ...
  if (colId === "state") {
    const v = entity?.state ?? null;
    const stateInt = toNumber(v);
    const label = stateInt !== null && STATE_LABELS[stateInt] ? STATE_LABELS[stateInt] : (v !== null ? String(v) : "-");
    if (mode === "badge") {
      return {
        type: "badge",
        value: label,
        params: {
          color: "primary",
          filterValue: stateInt !== null ? String(stateInt) : "",
          sortValue: stateInt !== null ? stateInt : 0,
        },
      };
    }
    return {
  ...
  if (colId === "description" || colId === "keyword") {
 ...
 */
export function adaptCampaignEntitiesTableResponse({ meta, entities }) {
  const ctx = { meta };
  const rows = (Array.isArray(entities) ? entities : []).map((entity) => {
    ...
    for (const colId of colIds) {
      cells[colId] = buildCampaignCell(colId, entity, ctx, { context: "table" });
    }
    return {
      id: entity?.id ?? null,
      cells,
