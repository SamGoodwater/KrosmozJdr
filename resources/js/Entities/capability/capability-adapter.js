/**
 * Capability adapter (Option B)
 *
 * @description
 * Transforme une réponse backend "entities" en `TableResponse` conforme à TanStackTable v2,
 * en générant les `cells` côté frontend à partir de `capability-descriptors`.
 */

import { DEFAULT_CAPABILITY_FIELD_VIEWS, getCapabilityFieldDescriptors } from "@/Entities/capability/capability-descriptors";
...
const capabilityShowHref = (id) => {
  try {
    return route("entities.capabilities.show", { capability: id });
  } catch {
    return null;
  }
};
...
const resolveViewConfigFor = (descriptor, { view = "table" } = {}) => {
  const views = descriptor?.display?.views || DEFAULT_CAPABILITY_FIELD_VIEWS;
...
/**
 * Construit une Cell v2 depuis une entité Capability brute.
 *
 * @param {string} colId
 * @param {any} entity
 * @param {any} ctx
 ...
 */
export function buildCapabilityCell(colId, entity, ctx = {}, opts = {}) {
  ...
  if (colId === "level" || colId === "pa" || colId === "po" || colId === "element") {
    const v = entity?.[colId] ?? null;
    const sortValue = colId === "level" || colId === "pa" ? (toNumber(v) ?? String(v ?? "")) : String(v ?? "");
    return {
      type: "text",
      value: v === null || typeof v === "undefined" || v === "" ? "-" : v,
      params: {
        sortValue,
        searchValue: colId === "element" ? (v === null || typeof v === "undefined" ? "" : String(v)) : undefined,
      },
    };
  }

  if (colId === "description" || colId === "effect") {
    const v = entity?.[colId] ?? null;
    return {
      type: "text",
 ...
 */
export function adaptCapabilityEntitiesTableResponse({ meta, entities }) {
  const ctx = { meta };
  const rows = (Array.isArray(entities) ? entities : []).map((entity) => {
    ...
    for (const colId of colIds) {
      cells[colId] = buildCapabilityCell(colId, entity, ctx, { context: "table" });
    }
    return {
      id: entity?.id ?? null,
      cells,
