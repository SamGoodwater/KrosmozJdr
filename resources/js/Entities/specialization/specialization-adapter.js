/**
 * Specialization adapter (Option B)
 *
 * @description
 * Transforme une réponse backend "entities" en `TableResponse` conforme à TanStackTable v2,
 * en générant les `cells` côté frontend à partir de `specialization-descriptors`.
 */

import { DEFAULT_SPECIALIZATION_FIELD_VIEWS, getSpecializationFieldDescriptors } from "@/Entities/specialization/specialization-descriptors";
...
const specializationShowHref = (id) => {
  try {
    return route("entities.specializations.show", { specialization: id });
  } catch {
    return null;
  }
};
...
const resolveViewConfigFor = (descriptor, { view = "table" } = {}) => {
  const views = descriptor?.display?.views || DEFAULT_SPECIALIZATION_FIELD_VIEWS;
...
/**
 * Construit une Cell v2 depuis une entité Specialization brute.
 *
 * @param {string} colId
 * @param {any} entity
 * @param {any} ctx
 ...
 */
export function buildSpecializationCell(colId, entity, ctx = {}, opts = {}) {
  ...
  if (colId === "name") {
    ...
    return {
      ...
      value: name,
      params: {
        href: specializationShowHref(id),
  ...
  if (colId === "capabilities_count") {
    const v = entity?.capabilities_count ?? null;
 ...
 */
export function adaptSpecializationEntitiesTableResponse({ meta, entities }) {
  const ctx = { meta };
  const rows = (Array.isArray(entities) ? entities : []).map((entity) => {
    ...
    for (const colId of colIds) {
      cells[colId] = buildSpecializationCell(colId, entity, ctx, { context: "table" });
    }
    return {
      id: entity?.id ?? null,
      cells,
