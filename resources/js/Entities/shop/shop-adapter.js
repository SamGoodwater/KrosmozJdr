/**
 * Shop adapter — Version simplifiée
 *
 * @description
 * Transforme une réponse backend "entities" en `TableResponse` conforme à TanStackTable v2.
 * 
 * ⚠️ IMPORTANT : Les cellules ne sont plus pré-générées ici. Elles sont générées à la volée
 * par le composant tableau via `shop.toCell()` selon la taille d'écran (xs-xl).
 *
 * @example
 * <EntityTanStackTable :response-adapter="adaptShopEntitiesTableResponse" />
 */

import { Shop } from "@/Models/Entity/Shop";

/**
 * Adapter: backend `{ meta, entities }` -> frontend `{ meta, rows }`
 *
 * @param {any} payload - Réponse backend avec meta et entities
 * @returns {{meta:any, rows:any[]}} Réponse formatée pour TanStackTable
 */
export function adaptShopEntitiesTableResponse(payload) {
  const meta = payload?.meta || {};
  const entities = Array.isArray(payload?.entities) ? payload.entities : [];

  // Transformer les entités brutes en instances de Shop
  const rows = entities.map((entityData) => {
    const shop = new Shop(entityData);
    
    return {
      id: shop.id,
      // Les cellules seront générées à la volée par le composant tableau via shop.toCell()
      // On ne pré-génère plus les cellules ici
      cells: {},
      rowParams: { 
        entity: shop, // Passer l'instance Shop pour génération des cellules
      },
    };
  });

  return { meta, rows };
}

export default adaptShopEntitiesTableResponse;
