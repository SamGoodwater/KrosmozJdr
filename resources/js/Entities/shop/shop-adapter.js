/**
 * Shop adapter (Option B)
 *
 * @description
 * Transforme une réponse backend "entities" en `TableResponse` conforme à TanStackTable v2,
 * en générant les `cells` côté frontend à partir de `shop-descriptors`.
 */

import { DEFAULT_SHOP_FIELD_VIEWS, getShopFieldDescriptors } from "@/Entities/shop/shop-descriptors";
...
const shopShowHref = (id) => {
  try {
    return route("entities.shops.show", { shop: id });
  } catch {
    return null;
  }
};
...
const resolveViewConfigFor = (descriptor, { view = "table" } = {}) => {
  const views = descriptor?.display?.views || DEFAULT_SHOP_FIELD_VIEWS;
...
/**
 * Construit une Cell v2 depuis une entité Shop brute.
 *
 * @param {string} colId
 * @param {any} entity
 * @param {any} ctx
 ...
 */
export function buildShopCell(colId, entity, ctx = {}, opts = {}) {
  ...
  if (colId === "npc_name") {
    const npc = entity?.npc || null;
    const creature = npc?.creature || null;
    const npcName = creature?.name || "-";
    return {
      type: "text",
      value: npcName,
      params: {
  ...
  if (colId === "price") {
    ...
    const priceNum = toNumber(v) ?? 0;
    return {
      type: "text",
      value: priceNum > 0 ? String(priceNum) : "-",
      params: {
        sortValue: priceNum,
      },
    };
 ...
 */
export function adaptShopEntitiesTableResponse({ meta, entities }) {
  const ctx = { meta };
  const rows = (Array.isArray(entities) ? entities : []).map((entity) => {
    ...
    for (const colId of colIds) {
      cells[colId] = buildShopCell(colId, entity, ctx, { context: "table" });
    }
    return {
      id: entity?.id ?? null,
      cells,
