/**
 * ShopFormConfig — Configuration des formulaires pour l'entité Shop
 *
 * @description
 * Configuration des formulaires d'édition (simple et bulk) pour les boutiques.
  ...
  formConfig
    ...
    .addField(
      new FormFieldConfig({
        ...
        .withGroup(descriptors.location?.edit?.form?.group || "Métier")
        .withPlaceholder(descriptors.location?.edit?.form?.placeholder || "Ex: Brakmar")
        .withShowInCompact(true)
        .withBulk(descriptors.location?.edit?.form?.bulk || {
          enabled: true,
          nullable: true,
    ...
    .addField(
      new FormFieldConfig({
        ...
        .withGroup(descriptors.price?.edit?.form?.group || "Métier")
        .withPlaceholder(descriptors.price?.edit?.form?.placeholder || "Ex: 100")
    ...
    .addField(
      new FormFieldConfig({
        key: "npc_id",
        type: "number",
        label: descriptors.npc_id?.edit?.form?.label || descriptors.npc_id?.label || "PNJ ID",
      })
        .withGroup(descriptors.npc_id?.edit?.form?.group || "Métier")
        .withShowInCompact(false)
        .withBulk(descriptors.npc_id?.edit?.form?.bulk || {
          enabled: true,
          nullable: true,
          build: (v) => (v === "" ? null : Number(v)),
        })
