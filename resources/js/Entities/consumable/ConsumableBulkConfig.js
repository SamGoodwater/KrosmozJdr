/**
 * ConsumableBulkConfig — Configuration de l'édition en masse (bulk) pour Consumable
 *
 * @description
 * Configuration de l'édition en masse pour les consommables.
 * Doit rester aligné avec le backend (bulk controller).
 * Utilise les descriptors simplifiés et CONSUMABLE_QUICK_EDIT_FIELDS.
 */

import { BulkConfig } from "../entity/BulkConfig.js";
import { getConsumableFieldDescriptors, CONSUMABLE_QUICK_EDIT_FIELDS } from "./consumable-descriptors.js";

/**
 * Crée la configuration bulk pour Consumable
 *
 * @param {Object} [ctx] - Contexte (permissions, etc.)
 * @returns {BulkConfig} Configuration bulk
 */
export function createConsumableBulkConfig(ctx = {}) {
  ...
  bulkConfig.withQuickEditFields(CONSUMABLE_QUICK_EDIT_FIELDS);
