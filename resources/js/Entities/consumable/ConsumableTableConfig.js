/**
 * ConsumableTableConfig — Configuration du tableau pour l'entité Consumable
 *
 * @description
 * Configuration du tableau TanStack pour les consommables.
...
/**
 * Crée la configuration du tableau pour Consumable
 *
 * @param {Object} [ctx] - Contexte (permissions, etc.)
 * @returns {TableConfig} Configuration du tableau
 */
export function createConsumableTableConfig(ctx = {}) {
  ...
  const tableConfig = new TableConfig({
    id: tableConfigData.id || "consumables.index",
    entityType: tableConfigData.entityType || "consumable",
  });
  ...
  // Colonne Consumable Type
  tableConfig.addColumn(
    new TableColumnConfig({
      key: "consumable_type",
      label: descriptors.consumable_type?.label || "Type",
      type: "badge",
      icon: descriptors.consumable_type?.icon || "fa-solid fa-tag",
    })
      .withOrder(6)
      .withSort(true)
      .withFilter({ id: "consumable_type_id", type: "multi" })
      .withFormat({
        xs: { mode: "badge" },
        sm: { mode: "badge" },
        md: { mode: "badge" },
  ...
  tableConfig.addColumn(
    new TableColumnConfig({
      ...
      type: "badge",
      icon: descriptors.rarity?.icon || "fa-solid fa-gem",
    })
      .withOrder(7)
      .withSort(true)
      .withFilter({
