/**
 * Attribute adapter (Option B)
 *
 * @description
 * Transforme une réponse backend "entities" en `TableResponse` conforme à TanStackTable v2,
 * en générant les `cells` côté frontend à partir de `attribute-descriptors`.
 */

import { DEFAULT_ATTRIBUTE_FIELD_VIEWS, getAttributeFieldDescriptors } from "@/Entities/attribute/attribute-descriptors";
...
const attributeShowHref = (id) => {
  try {
    return route("entities.attributes.show", { attribute: id });
  } catch {
    return null;
  }
};
...
const resolveViewConfigFor = (descriptor, { view = "table" } = {}) => {
  const views = descriptor?.display?.views || DEFAULT_ATTRIBUTE_FIELD_VIEWS;
...
/**
 * Construit une Cell v2 depuis une entité Attribute brute.
 *
 * @param {string} colId
 * @param {any} entity
 * @param {any} ctx
 ...
 */
export function buildAttributeCell(colId, entity, ctx = {}, opts = {}) {
 ...
 */
export function adaptAttributeEntitiesTableResponse({ meta, entities }) {
  const ctx = { meta };
  const rows = (Array.isArray(entities) ? entities : []).map((entity) => {
    ...
    for (const colId of colIds) {
      cells[colId] = buildAttributeCell(colId, entity, ctx, { context: "table" });
    }
    return {
      id: entity?.id ?? null,
      cells,
