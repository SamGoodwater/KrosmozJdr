/**
 * CreatureTrait field descriptors — Version simplifiée
 *
 * @description
 ...
 * const descriptors = getCreatureTraitFieldDescriptors({ meta });
 */

import { getEntityStateOptions, getUserRoleOptions } from "@/Utils/Entity/SharedConstants";

/**
 * @typedef {Object} CreatureTraitFieldDescriptor
 ...
 * @returns {Record<string, CreatureTraitFieldDescriptor>} Objet avec tous les descripteurs
 */
export function getCreatureTraitFieldDescriptors(ctx = {}) {
  ...
  return {
    id: {
      ...
      visibleIf: (context = ctx) => Boolean(context?.capabilities?.updateAny ?? context?.meta?.capabilities?.updateAny ?? false),
      table: {
        sortable: true,
