...
trait HasMediaCustomNaming
{
    ...
    public function getMediaFileNameForCollection(string $collection, string $extension = ''): ?string
    {
        ...
        $uniqid = uniqid('', false);
        $slugSource = (isset($this->attributes['slug']) && (string) $this->attributes['slug'] !== '')
            ? (string) $this->attributes['slug']
            : (string) ($this->name ?? $this->getKey());
        $slug = Str::slug($slugSource);
        $keyRaw = isset($this->attributes['key']) ? (string) $this->attributes['key'] : '';
        $key = $keyRaw !== '' ? Str::slug($keyRaw) : Str::slug((string) $this->getKey());
