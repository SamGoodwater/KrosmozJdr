...
| Fichier | Rôle |
|--------|------|
| **resources/scrapping/config/sources/dofusdb/item-types.json** | Catalogue de référence : liste des 232 item-types (id, superTypeId, categoryId, nameFr). Utilisé pour validation et pour s’assurer qu’aucun type n’est oublié. |
...
file_put_contents("resources/scrapping/config/sources/dofusdb/item-types.json", json_encode($out, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
