# Index

Index de **jeu** : où trouver une règle à la table. Ce n’est pas une table des matières mot à mot.

**Navigation** : [Table des matières](TABLE_DES_MATIERES.md) · [Clés caractéristiques](REFERENCE_CLES_CARACTERISTIQUES.md) · [Progression (table unique)](2-Creer-un-personnage/2.3-choisir-sa-classe/2.3.2-systeme-de-progression.md) · [Récap](RECAP.md)

Les numéros renvoient aux fichiers du livre. Changelog et historique : [[kref:page:changelog|Changelog]] (Informations).

---

## Combat et tour

- **Actions bonus** (2–3 PA, 1×/round) : 3.2.2
- **Avantage / désavantage** : 1.2.1
- **CA** (classe d’armure) : 2.2.2, 3.2.2
- **Combat non significatif** : 3.3.4
- **Critique** (nat. 20, dés doublés sauf fiche) : 3.2.2 · rappel 1.2.1
- **d20 + DD 15** : 1.2.1
- **Esquiver** (3 PA + 2 PM, unique) : 3.2.2
- **Fuite** : 3.2.3
- **Grille / diagonale** (1 PM = 1,5 m / 1 case) : 1.2.2, 3.2.2
- **Initiative** (1d20 + Intelligence) : 2.2.2, 3.2.1
- **Mains nues** : 2.6.1
- **PA légendaires** (boss) : 3.2.2, 5.1.2
- **Réaction** (1 / round, 2–3 PA) : 3.2.3
- **Round / tour** (6 s) : 1.2.3, 3.2.2
- **Tacle** (auto, hors diagonale) : 3.2.3
- **Touche vs sauvegarde** : 3.3.2 · 3.2.2

## Ressources (PA, PM, PO, Wakfu, PV)

- **PA** (6 / max 12) : 2.2.2, 1.2.2
- **PM** (3 / max 6) : 2.2.2, 1.2.2
- **PO** (portée, 0 / max 6) : 2.2.2, 1.2.2
- **PV** (formule seeder `life_points_creature`) : 2.2.2, 3.2.4
- **Repos** (1 court entre deux longs ; long 8 h) : 3.2.4
- **Wakfu** (hors combat ; 1 pt = 1× PA max) : 3.3.4
- **Mort** (jets 10+, 3/3) : 3.2.4

## Création et progression

- **Aptitudes** (passives, automatiques aux niv. 3 / 9 / 15) : 2.4.3 · jeu : 3.4.1
- **Capacités** (actives, 1 garantie par palier + emplacement libre) : 2.4.4 · activation : 3.4.2
- **Caractéristiques** (scores, mods, plafond = 14 + ⌊niv/2⌋) : 2.2.1, 2.2.3
- **Classes** (19) : 2.3.1
- **Compétences** (liste unique ; 3 au niv. 1 puis 1 par palier, 2 si la fiche le dit) : 3.5.1, 2.4.5
- **Intensification** (paliers 13 / 16 / 20) : 3.3.6
- **Emplacement libre** (capacité / +2 points de carac / trait au niv. 12+) : 2.4.2
- **Expertise** (paliers 9 / 15 / 20, max 3) : 3.5.3, 2.4.5
- **Maîtrise** (1 + ⌊niveau/4⌋) : 2.3.2, 3.5.3
- **Passifs de classe** : 2.3.4
- **Spécialisations** (6 jouables / 11 prévues ; paliers 1, 3, 6, 9, 12, 15, 20) : 2.4.1, 2.4.2
- **Table de progression** (12 sorts au niv. 12 / intensification 13·16·20 / 6 points fixes / 7 paliers de spé) : 2.3.2
- **Voies élémentaires** : 2.3.3

## Équipement

- **Bonus d'équipement** (**+8** au total par carac principale, dont **+2** de forgemagie ; +4 par objet) : 2.2.3, 2.6.1
- **Forgemagie** : 4.3.4, 2.6.1
- **Panoplies** : 2.6.2, 5.2.4
- **Prix kamas** : 2.6.1, 4.1.3, 5.3.5
- **Slots** (chapeau, cape, ceinture, bottes, amulette, anneaux, bouclier) : 2.6.1

## Sorts, pièges, exploration

- **Glyphes / pièges de sort** : 3.3.2
- **Invocations** : 3.3.3
- **Piège de donjon** : 3.1.3
- **Portée et ligne de vue** : 3.3.2
- **Rythme de marche** : 3.1.1
- **Intensification** (paliers 13 / 16 / 20) : 3.3.6
- **Sorts (lancement)** : 3.3.2

## MJ

- **Agressivité** : 3.2.1, 4.2.4
- **Créer un PNJ / monstre** : 5.1.2
- **DD (−5 / +5, pas de jet si trivial)** : 1.2.1
- **Rencontres / budget** : 5.1.3, 5.3.4
- **Trésor / rareté** : 5.1.4, 5.3.2
- **XP / rythme de niveau** : 5.2.5

## Monde

- **Kamas** : 4.1.3
- **Langues** : 4.1.4
- **Métiers** : 4.3
- **Monde des Douze** : 4.1.1
