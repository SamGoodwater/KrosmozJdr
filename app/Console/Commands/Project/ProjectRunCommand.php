...
class ProjectRunCommand extends Command
{
    ...
    protected function collectActions()
    {
        ...
        // 0. RESET (délégation à setup --clean / --refresh)
        if ($this->option('reset:pnpm')) {
            $actions[] = 'runSetupRefresh';
        }
        if ($this->option('reset:composer')) {
            $actions[] = 'runSetupRefresh';
        }
        if ($this->option('reset:all')) {
            $actions[] = 'resetAll';
        }
        if ($this->option('reset:full')) {
            $actions[] = 'resetFull';
        }
        // 1. KILL
        if ($this->option('kill') || $this->option('regenerate') || $this->option('all')) {
            $actions[] = 'killServers';
        }
        ...
        if ($this->option('clear:all') || $this->option('regenerate') || $this->option('all')) {
            ...
            if ($this->option('clear:view')) {
                $actions[] = 'clearView';
            }
            if ($this->option('clear:debugbar')) {
                $actions[] = 'clearDebugbar';
            }
            if ($this->option('clear:queue')) {
                $actions[] = 'clearQueue';
            }
            if ($this->option('clear:schedule')) {
                $actions[] = 'clearSchedule';
            }
            if ($this->option('clear:event')) {
                $actions[] = 'clearEvent';
            }
            if ($this->option('clear:optimize')) {
                $actions[] = 'clearOptimize';
            }
        }

        // 3. UPDATE/INSTALL (délégation à setup pour apt/pnpm/composer)
        if ($this->option('update:all') || $this->option('all')) {
            ...
            if ($this->option('update:system') || $this->option('update:pnpm') || $this->option('update:composer')) {
                $actions[] = 'runSetupUpdate';
            }
            if ($this->option('install:pnpm') || $this->option('install:composer')) {
                $actions[] = 'runSetupInstall';
            }
            if ($this->option('update:css')) {
                $actions[] = 'updateCss';
            }
            if ($this->option('update:docs')) {
                $actions[] = 'updateDocs';
            }
            if ($this->option('update:privilege')) {
        ...
        if ($this->option('optimise:all') || $this->option('regenerate') || $this->option('all')) {
            ...
            if ($this->option('optimise:ide')) {
                $actions[] = 'optimiseIde';
            }
            if ($this->option('optimise:laravel')) {
                $actions[] = 'optimiseLaravel';
            }
        }

        // 5. MIGRATE (délégation à setup --db)
        if ($this->option('migrate') || $this->option('update:all') || $this->option('update:base') || $this->option('regenerate') || $this->option('all')) {
            $actions[] = 'runSetupDb';
        }

        // 6. QUALITY CHECKS
        if ($this->option('check:effects-quality')) {
            $actions[] = 'checkEffectsQuality';
        }
        if ($this->option('check:effects-quality:dev')) {
            $actions[] = 'checkEffectsQualityDev';
        }
        if ($this->option('pipeline:effects-quality')) {
            $actions[] = 'pipelineEffectsQuality';
        }
        if ($this->option('pipeline:effects-quality:dev')) {
            $actions[] = 'pipelineEffectsQualityDev';
        }

        // 7. DEV
        if ($this->option('dev') || $this->option('all')) {
            $actions[] = 'runDev';
        }
        if ($this->option('dev:watch')) {
            $actions[] = 'runDevWatch';
        }
    ...
    protected function clearCache(): void
    {
        try {
            ...
            if (str_contains($msg, 'could not find driver') || str_contains($msg, 'Connection:')) {
                $this->warn('Cache non vidé : driver BDD indisponible (ex. extension pdo_mysql manquante).');
                $this->warn('Installez php-mysql ou définissez CACHE_STORE=file dans .env.');
            } else {
                throw $e;
            }
        }
    ...
    protected function clearDebugbar()
    {
    ...
    protected function clearOptimize()
    {
        $this->call('optimize:clear');
    }

    protected function runSetupInstall(): void
    {
        $this->call('setup', ['--install' => true]);
    }

    protected function runSetupUpdate(): void
    {
        $this->call('setup', ['--update' => true]);
    }

    protected function runSetupDb(): void
    {
        $this->call('setup', ['--db' => true]);
    }
