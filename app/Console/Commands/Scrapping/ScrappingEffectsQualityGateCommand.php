...
final class ScrappingEffectsQualityGateCommand extends Command
{
    ...
    protected $description = 'Gate de qualité des effets de sorts (utile en CI)';
    ...
    private function buildConversionSummary(SpellEffectConversionFormulaResolver $formulaResolver): array
    {
        $query = DB::table('effect_sub_effect as es')
            ...
            ->whereExists(function ($q): void {
                $q->select(DB::raw(1))
                    ->from('effect_degrees as ed')
                    ->join('effect_spell as esp', 'esp.effect_id', '=', 'ed.effect_id')
                    ->whereColumn('ed.id', 'es.effect_degree_id');
            })
            ->orderBy('es.id')
