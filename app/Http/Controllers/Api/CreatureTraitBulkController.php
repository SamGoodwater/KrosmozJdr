 ...
 *
 * @example
 * PATCH /api/entities/creature-traits/bulk
 * { "ids":[1,2,3], "state":"playable", "read_level":0, "write_level":3, "description":"Nouvelle description" }
 */
class CreatureTraitBulkController extends Controller
{
    public function bulkUpdate(Request $request): JsonResponse
    {
        $this->authorize('updateAny', CreatureTrait::class);

        $validated = $request->validate([
            'ids' => ['required', 'array', 'min:1'],
            'ids.*' => ['integer', 'min:1', 'exists:creature_traits,id'],
        ...
        DB::beginTransaction();
        try {
            $models = CreatureTrait::query()->whereIn('id', $ids)->get();
