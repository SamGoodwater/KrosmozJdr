...
/**
 * API Bulk update pour les spécialisations.
 *
 * @description
 * Applique un patch sur une liste d'IDs (sélection multiple). Seuls les champs fournis sont modifiés.
 *
 * @example
 * PATCH /api/entities/specializations/bulk
 * { "ids":[1,2,3], "usable":true, "is_visible":"guest", "description":"Nouvelle description" }
 */
class SpecializationBulkController extends Controller
{
    public function bulkUpdate(Request $request): JsonResponse
    {
        $this->authorize('updateAny', Specialization::class);
        ...
        DB::beginTransaction();
        try {
            $models = Specialization::query()->whereIn('id', $ids)->get();
