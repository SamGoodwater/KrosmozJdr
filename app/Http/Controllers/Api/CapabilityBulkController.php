...
/**
 * API Bulk update pour les capacités.
 *
 * @description
...
class CapabilityBulkController extends Controller
{
    ...
    {
        $this->authorize('updateAny', Capability::class);

        $validated = $request->validate([
            'ids' => ['required', 'array', 'min:1'],
            'ids.*' => ['integer', 'min:1', 'exists:capabilities,id'],
            ...
            'effect' => ['sometimes', 'nullable', 'string'],
        ...
        DB::beginTransaction();
        try {
            $models = Capability::query()->whereIn('id', $ids)->get();
