...
/**
 * API Bulk update pour les sorts.
 *
 * @description
 * Applique un patch sur une liste d'IDs (sélection multiple). Seuls les champs fournis sont modifiés.
 *
 * @example
 * PATCH /api/entities/spells/bulk
 * { "ids":[1,2,3], "state":"playable", "read_level":0, "write_level":3, "auto_update":false, "level":"50", "pa":"3" }
 */
class SpellBulkController extends Controller
{
    public function bulkUpdate(Request $request): JsonResponse
    {
        $this->authorize('updateAny', Spell::class);

        $validated = $request->validate([
            'ids' => ['required', 'array', 'min:1'],
            'ids.*' => ['integer', 'min:1', 'exists:spells,id'],
            ...
            'po_min' => ['sometimes', 'nullable', 'string', 'max:64'],
            'po_max' => ['sometimes', 'nullable', 'string', 'max:64'],
        ...
        DB::beginTransaction();
        try {
            $models = Spell::query()->whereIn('id', $ids)->get();
