 ...
 *
 * @example
 * PATCH /api/entities/conditions/bulk
 * { "ids":[1,2,3], "state":"playable", "read_level":0, "write_level":3, "description":"Nouvelle description" }
 */
class ConditionBulkController extends Controller
{
    public function bulkUpdate(Request $request): JsonResponse
    {
        $this->authorize('updateAny', Condition::class);

        $validated = $request->validate([
            'ids' => ['required', 'array', 'min:1'],
            'ids.*' => ['integer', 'min:1', 'exists:conditions,id'],
        ...
        DB::beginTransaction();
        try {
            $models = Condition::query()->whereIn('id', $ids)->get();
