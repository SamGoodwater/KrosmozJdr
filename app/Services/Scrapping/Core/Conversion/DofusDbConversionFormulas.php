 ...
 * DofusDbConversionFormulaService (BDD) : si conversion_formula est renseignée,
 * elle est évaluée via FormulaEvaluator (variable [d] = valeur Dofus, [level] = niveau JDR pour vie).
 * Sinon fallback sur formula_type/parameters ou config/dofusdb_conversion.php.
 *
 * @see config/dofusdb_conversion.php
...
final class DofusDbConversionFormulas
{
    ...
    /**
     * Retourne la config de conversion DofusDB (mapping, limites).
     *
     * @return array<string, mixed>
     */
    public function getConversionConfig(): array
    {
        return Config::get('dofusdb_conversion', []);
    }
    ...
    public function convertLevel(int|float|null $dofusdbValue, string $entityType = 'monster'): int
    {
        ...
        if ($conversionFormula !== null) {
            ...
            if ($formula !== null && $formula['formula_type'] === 'linear') {
                ...
                $config = Config::get('dofusdb_conversion.formulas.level', []);
    ...
    public function convertLife(int|float|null $dofusdbValue, int $krosmozLevel, string $entityType = 'monster'): int
    {
        ...
        if ($conversionFormula !== null) {
            ...
            if ($formula !== null && $formula['formula_type'] === 'linear_with_level') {
                ...
                $config = Config::get('dofusdb_conversion.formulas.life', []);
    ...
    public function convertAttribute(string $characteristicId, int|float|string|null $dofusdbValue, string $entityType = 'monster'): int
    {
        ...
        if ($formula !== null && $formula['formula_type'] === 'sqrt_attribute') {
            ...
            $config = Config::get('dofusdb_conversion.formulas.attributes', []);
    ...
    {
        $ids = Config::get('dofusdb_conversion.formulas.attribute_ids', ['strength', 'intelligence', 'chance', 'agility']);
    ...
    public function convertInitiative(int|float|null $dofusdbValue, string $entityType = 'monster'): int
    {
        ...
        if ($formula !== null && $formula['formula_type'] === 'ratio_initiative') {
            ...
            $config = Config::get('dofusdb_conversion.formulas.initiative', []);
     ...
     *
     * Utilise config/dofusdb_conversion.php (element_id_to_resistance) si présent, sinon fallback.
    ...
    {
        $limitsSource = Config::get('dofusdb_conversion.limits_source', 'characteristics');
        if ($limitsSource !== 'characteristics') {
            $local = Config::get("dofusdb_conversion.limits.{$entityType}", []);
     ...
     * Utilise config/dofusdb_conversion.php (effect_id_to_characteristic).
     *
