...
final class IntegrationService
{
    ...
    private function integrateMonster(array $convertedData, array $options = []): IntegrationResult
    {
        ...
        try {
            ...
            Log::info('Intégration V2 monstre', [
                'creature_id' => $creature->id,
            ...
            DB::rollBack();
            Log::error('Erreur intégration V2 monstre', ['error' => $e->getMessage()]);
    ...
    private function integrateSpell(array $convertedData, array $options = []): IntegrationResult
    {
        ...
        try {
            ...
            DB::commit();
            Log::info('Intégration V2 sort', ['spell_id' => $spell->id, 'action' => $action]);
            ...
            DB::rollBack();
            Log::error('Erreur intégration V2 sort', ['error' => $e->getMessage()]);
    ...
    private function integrateClass(array $convertedData, array $options = []): IntegrationResult
    {
        ...
        try {
            ...
            DB::commit();
            Log::info('Intégration V2 classe', ['class_id' => $class->id, 'action' => $action]);
            ...
            DB::rollBack();
            Log::error('Erreur intégration V2 classe', ['error' => $e->getMessage()]);
    ...
    private function integrateItem(array $convertedData, array $options = []): IntegrationResult
    {
        ...
        try {
            ...
            DB::commit();
            Log::info('Intégration V2 item', ['id' => $entity->id, 'table' => $targetTable, 'action' => $action]);
            ...
            DB::rollBack();
            Log::error('Erreur intégration V2 item', ['error' => $e->getMessage()]);
