 ...
 *
     * Expose les formules par (characteristic_key, entity) pour que DofusDbConversionFormulas
...
final class DofusDbConversionFormulaService
{
    ...
    public function getConversionFormula(string $characteristicKey, string $entity): ?string
    {
        ...
        $byChar = $all[$characteristicKey] ?? null;
        if ($byChar === null) {
            return null;
        }
     ...
     */
    public function getFormula(string $characteristicKey, string $entity): ?array
    {
        $all = $this->getAllFormulas();
        $byChar = $all[$characteristicKey] ?? null;
        if ($byChar === null) {
     ...
     * @return string|null
     */
    public function getHandlerName(string $characteristicKey, string $entity): ?string
    {
        $def = $this->getFormula($characteristicKey, $entity);
    ...
    private function buildAllFormulas(): array
    {
        ...
        $out = [];
        foreach ($rows as $row) {
            $out[$row->characteristic_key][$row->entity] = [
